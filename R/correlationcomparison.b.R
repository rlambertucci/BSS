  ...
  private = list(
    ...
    .init = function() {
      info_html <- paste0(
        ...
        '<ul>',
        '<li><b>Independent Comparison</b> &rarr; Tests whether two correlation coefficients ',
        'obtained from <b>two independent groups</b> are significantly different from each other. ',
        'Uses Fisher\'s Z-transformation to compare r<sub>xy</sub> in Group 1 vs. r<sub>xy</sub> in Group 2. ',
        '<i>Fisher, R. A. (1921).</i></li>',
        '<li><b>Dependent Comparison</b> &rarr; Tests whether two correlation coefficients ',
        'obtained from <b>the same group</b> are significantly different, where both correlations ',
        'share a common variable. Compares r<sub>12</sub> vs. r<sub>13</sub> while accounting for ',
        'the intercorrelation r<sub>23</sub> between the non-shared variables. ',
        '<i>Steiger, J. H. (1980).</i></li>',
        '</ul>',
        '<p style="font-style: italic; padding-top: 10px;">',
        'Note: For both methods, sample size must be at least 4 and correlation coefficients ',
        'must be strictly between -1 and 1.',
        '</p>',
        '</div>'
      )
      self$results$comparisonInfo$setContent(info_html)
    },
    ...
    .run = function() {
      ...
      if (mode == "independent") {
        ...
        table$setNote(
          "citation",
          "Note. Fisher's Z-transformation is used to compare two independent correlation coefficients. Citation: Fisher, R. A. (1921). On the 'probable error' of a coefficient of correlation. Metron, 1, 3-32."
        )
        ...
        table$setNote(
          "citation",
          "Note. Steiger's Z-test / Williams' t-test modification of Hotelling's test is used to compare two dependent correlation coefficients sharing a common variable. Citations: (1) Steiger, J. H. (1980). Tests for comparing elements of a correlation matrix. Psychological Bulletin, 87(2), 244-251. (2) Williams, E. J. (1959). The comparison of regression variables. Journal of the Royal Statistical Society: Series B, 21(2), 396-399."
        )
        ...
        if (calculation_possible) {
          ...
          if (det_R <= 1e-10) { 
            ...
            denominator_term = (2 * ( (n - 1)/(n - 3) ) * det_R + r_bar_sq * (1 - r23)^3)
