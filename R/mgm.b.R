mgmClass <- if (requireNamespace('jmvcore')) R6::R6Class(
  ...
  private = list(
    ...
    .run = function() {
      ...
      if (!is.null(nodeGroupVarName)) {
        tryCatch({
          ...
          # Filtra apenas valores não vazios para evitar erro
          raw_labels <- raw_labels[!is.na(raw_labels) & raw_labels != ""]
          
          # Mapeamento: Variável -> Grupo
          ...
          if (matched_any) {
            # Converte para lista nomeada de índices para o qgraph
            unique_groups <- unique(na.omit(node_group_mapping))
            qgraph_groups <- list()
            for (g in unique_groups) {
              # qgraph precisa dos índices numéricos referentes à ordem das variáveis na matriz
              indices <- which(node_group_mapping == g)
              qgraph_groups[[g]] <- indices
            }
          }
        }, error = function(e) {
          # Falha silenciosa no parsing da legenda
          qgraph_groups <- NULL
        })
      }
      # -----------------------------------------------------
      ...
      for (groupName in groupLevels) {
        ...
        tryCatch({ 
          ...
          # --- NOVO CÁLCULO DE CLUSTERING (Barrat, Onnela, WS, Zhang) ---
          clusteringDataNorm <- NULL
          tryCatch({
            clust_res <- qgraph::clustcoef_auto(adjMatrix) # Usa a matriz de adjacência calculada
            ...
            warning(paste("Clustering calculation failed for group", groupName, ":", e$message))
          })
          # -------------------------------------------------------------
      ...
      clusteringTable <- self$results$clustering; clusteringTable$setState(NULL); allClusteringDataForPlot <- list(); 
      if (isTRUE(self$options$tableClustering) || isTRUE(self$options$plotClustering)) { 
        ...
        for (groupName in successful_groups) { 
          ...
          if (!is.null(clusteringData_raw) && nrow(clusteringData_raw) > 0) { 
            ...
            # Normalização Z-score para as 4 medidas
            ...
            if (isTRUE(self$options$tableClustering)) { 
            ...
            allClusteringDataForPlot[[groupName]] <- NULL 
          } 
        }; 
        if (data_found_for_clust_table && isTRUE(self$options$tableClustering)) { 
          ...
          clusteringTable$setNote("clustering_info", NULL) 
        } 
      }
      
      if (isTRUE(self$options$plotNetwork) && length(weightedAdjMatrices) > 0) { 
        message("Preparing state for Network Plot"); 
        commonLayout <- NULL; 
        graphs_for_layout <- weightedAdjMatrices[successful_groups]; 
        if (length(graphs_for_layout) > 1) { 
          tryCatch({ commonLayout <- qgraph::averageLayout(graphs_for_layout, layout = self$options$layoutAlgorithm); if (!is.null(commonLayout) && !is.null(allVarNames) && length(allVarNames) == nrow(commonLayout)) rownames(commonLayout) <- allVarNames }, error = function(e) { warning(paste("Could not compute average layout:", e$message)); commonLayout <- NULL }) 
        }; 
        if (is.null(commonLayout) && length(graphs_for_layout) > 0) { 
          first_graph_name <- names(graphs_for_layout)[1]; 
          tryCatch({ q_obj <- qgraph::qgraph(graphs_for_layout[[first_graph_name]], layout = self$options$layoutAlgorithm, DoNotPlot = TRUE); commonLayout <- q_obj$layout; if (!is.null(commonLayout) && !is.null(allVarNames) && length(allVarNames) == nrow(commonLayout)) rownames(commonLayout) <- allVarNames }, error = function(e) { warning(paste("Layout for first graph failed:", e$message, "- using circle.")); tryCatch({ q_obj_circ <- qgraph::qgraph(graphs_for_layout[[first_graph_name]], layout = "circle", DoNotPlot = TRUE); commonLayout <- q_obj_circ$layout; if (!is.null(commonLayout) && !is.null(allVarNames) && length(allVarNames) == nrow(commonLayout)) rownames(commonLayout) <- allVarNames }, error = function(e2) { warning(paste("Circle layout fallback also failed:", e2$message)); commonLayout <- NULL }) }) 
        }; 
        plotStateNetwork <- list(
          ...
          layout = commonLayout, 
          labels = if(isTRUE(self$options$showLabels)) allVarNames else FALSE, 
          groupNames = successful_groups, 
          layoutAlgorithm = self$options$layoutAlgorithm,
          nodeGroups = qgraph_groups # Adicionado
        ); 
        self$results$networkPlot$setState(plotStateNetwork) 
      }
    ...
    .plotNetwork = function(image, ggtheme, theme, ...) { 
      ...
      if (!requireNamespace("qgraph", quietly = TRUE)) { plot(1, type="n", axes=FALSE, xlab="", ylab=""); text(1, 1, "qgraph package missing.", col="red"); return(FALSE) }; 
      ...
      nodeGroups <- plotState$nodeGroups; # Adicionado
      ...
      for (i in 1:numPlots) { 
        ...
        }, error = function(e) { warning(paste("Network plotting failed for group", groupName, ":", e$message)); plot(1, type="n", axes=FALSE, xlab="", ylab=""); text(1, 1, paste("Plot Error (", groupName, ")"), col="red", cex=0.8); plotOk <<- FALSE }) 
      }; 
      return(plotOk) 
    },
    ...
    .plotClustering = function(image, ggtheme, theme, ...) { 
      ...
      if (length(measure_vars_clust) == 0) { warning("No clustering measures (Barrat, Onnela, WS, Zhang) found in plot data."); return(FALSE) }; 
