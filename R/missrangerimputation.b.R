  ...
  private = list(
    ...
    .init = function() {
      info_html <- paste0(
        ...
        '<li><b>Multiple Imputation (Ensemble)</b> &rarr; Executes the algorithm N times. To ensure ',
        'forest diversity while maintaining full reproducibility, a deterministic seed is applied to ',
        'each iteration (<i>Seed + 1, Seed + 2, ..., Seed + N</i>). The final result is the <b>Mean</b> ',
        'for continuous variables and the <b>Mode</b> for categorical variables across all iterations.</li>',
        '</ul>',
    ...
    .run = function() {
      ...
      use_mi                <- self$options$use_multiple_imputation
      ...
      # --- GERAÇÃO DOS GRÁFICOS (Antes do Run Imputation) ---
      ...
      # PARA SE A CHECKBOX NÃO ESTIVER MARCADA
      ...
      imputed_data_result <- tryCatch({
        ...
        m <- if (isTRUE(use_mi)) num_imputations else 1
        imp_list <- list()
        
        for (i in 1:m) {
          # Incrementa a semente para garantir diversidade nas árvores, mas mantendo reprodutibilidade
          if (!is.null(base_seed)) set.seed(base_seed + i)
        ...
        for (var in names(imp_list[[1]])) {
          if (is.numeric(data_to_impute[[var]])) {
            # Média das imputações numéricas
            mat <- sapply(imp_list, function(df) df[[var]])
            agg_res[[var]] <- rowMeans(mat, na.rm = TRUE)
          } else {
            # Moda das imputações categóricas
            mat <- sapply(imp_list, function(df) as.character(df[[var]]))
            agg_res[[var]] <- apply(mat, 1, get_mode)
            # Restaura os níveis do Fator
        ...
        agg_res # Retorna o dataset agregado
      ...
      if (total_na_original > 0) {
        imp_type_text <- if (isTRUE(use_mi)) paste0("Multiple imputation (Ensemble of ", num_imputations, " datasets) completed. ") else "Single imputation completed. "
        self$results$text$setContent(paste0(imp_type_text, total_na_original, " NA(s) handled."))
      } else {
      ...
      if (self$results$imputedColsOutput$isNotFilled() && !is.null(imputed_data_result)) {
        ...
        for (var_name_key in vars_to_impute) {
          if (var_name_key %in% names(imputed_data_result)) {
            ...
            output_titles <- c(output_titles, paste0(var_name_key, "_imputed")) 
