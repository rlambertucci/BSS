  ...
  private = list(
    ...
    .init = function() {
      ...
      if (is.null(self$results$instructions)) {
        self$results$insert(1, createJaspResultsElement(name='instructions', title=' ', type='Preformatted'))
      }
    ...
    .run = function() {
          ...
          for (col in cols_to_norm_cent) {
            if (col %in% names(centralityDataNorm)) { 
              ...
              if (length(valid_vals) > 1) {
                mean_val <- mean(valid_vals); sd_val <- stats::sd(valid_vals)   
      ...
      if (isTRUE(self$options$plotNetwork) && length(networkGraphsList) > 0) { commonLayout <- NULL; tryCatch({ graphs_for_layout <- networkGraphsList[successful_groups]; if (length(graphs_for_layout) > 0) { commonLayout <- qgraph::averageLayout(graphs_for_layout, layout = self$options$layoutAlgorithm); if (!is.null(commonLayout)) rownames(commonLayout) <- nodeLabels } }, error = function(e) { jmvcore::reject(paste("Layout calculation failed:", e$message), code="layout_error"); tryCatch({ commonLayout <- qgraph::averageLayout(networkGraphsList[[1]], layout = "circle"); if (!is.null(commonLayout)) rownames(commonLayout) <- nodeLabels }, error=function(e2){}) }); plotState <- list(graphs = networkGraphsList, layout = commonLayout, labels = if(isTRUE(self$options$showLabels)) nodeLabels else FALSE, groupNames = successful_groups); self$results$networkPlot$setState(plotState) }
    ...
    .plotNetwork = function(image, ggtheme, theme, ...) {
      ...
      plotOk <- TRUE
      for (i in 1:numPlots) { groupName <- groupNames[i]; currentGraph <- graphsToPlot[[groupName]]; if (is.null(currentGraph)) next; qArgs <- list(input = currentGraph, layout = layout, labels = labels, theme = "colorblind", DoNotPlot = FALSE, title = groupName); tryCatch({ qgraph::qgraph(qArgs$input, layout=qArgs$layout, labels=qArgs$labels, theme=qArgs$theme, title=qArgs$title) }, error = function(e) { jmvcore::reject(paste("Network plotting failed for group", groupName, ":", e$message), code = "network_plot_error"); plot(1, type="n", axes=FALSE, xlab="", ylab=""); text(1, 1, paste("Plot Error (", groupName, "):\n", e$message), col="red"); plotOk <<- FALSE }) }
      return(plotOk)
    }, 
