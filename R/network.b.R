NetworkClass <- if (requireNamespace('jmvcore')) R6::R6Class(
  ...
  private = list(
    ...
    .run = function() {
      ...
      for (groupName in groupLevels) {
        ...
        tryCatch({
          ...
          barrat_raw <- suppressWarnings(igraph::transitivity(igraph_obj, type = "barrat", weights = abs(edge_weights))) 
          # onnela_raw <- suppressWarnings(igraph::transitivity(igraph_obj, type = "weighted", weights = abs(edge_weights))) # REMOVED
          ws_raw     <- suppressWarnings(igraph::transitivity(igraph_obj, type = "local", weights = edge_weights))
          clusteringDataRaw <- data.frame(node = nodeLabels, Barrat = barrat_raw, WS = ws_raw) # MODIFIED: Onnela removed
          rownames(clusteringDataRaw) <- NULL; clusteringDataNorm <- clusteringDataRaw
          cols_to_norm_clust <- c("Barrat", "WS") # MODIFIED: Onnela removed
          for (col in cols_to_norm_clust) {
            if (col %in% names(clusteringDataNorm)) {
              ...
              if (length(valid_vals) > 1) {
                ...
                else { finite_indices <- which(is.finite(clusteringDataNorm[[col]])); if (length(unique(valid_vals)) == 1) { clusteringDataNorm[[col]][finite_indices] <- 0 } else { clusteringDataNorm[[col]][finite_indices] <- centralityDataNorm[[col]][finite_indices] - mean_val } }
      ...
      if (isTRUE(self$options$tableClustering) || isTRUE(self$options$plotClustering)) { 
        ...
        required_cols <- c("group", "node", "Barrat", "WS"); # MODIFIED: Onnela removed
          ...
          if (!is.null(clusteringData)) { 
            ...
            if (isTRUE(self$options$tableClustering) && nrow(clusteringData) > 0) { 
              for (i in 1:nrow(clusteringData)) { 
                ...
                row_values <- list(node = clusteringData$node[i], Barrat = clusteringData$Barrat[i], WS = clusteringData$WS[i]); # MODIFIED: Onnela removed
    ...
    .plotClustering = function(image, ggtheme, theme, ...) {
      ...
      measure_order <- c("Barrat", "WS"); # MODIFIED: Onnela removed
