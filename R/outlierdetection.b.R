  ...
  private = list(
    .run = function() {
      ...
      for (var in self$options$vars) {
        ...
        tryCatch({
          if (has_group) {
            # Verifica se TODAS as variáveis de agrupamento existem nos dados
            if (!all(group_var %in% names(self$data))) {
              stop("One or more grouping variables not found in data.")
            }
            
            # Se houver mais de uma variável de agrupamento, cria uma interação entre elas
            if (length(group_var) > 1) {
              group_data <- self$data[, group_var, drop = FALSE]
              combined_group <- interaction(group_data, drop = TRUE, sep = " - ")
            } else {
              combined_group <- as.factor(self$data[[group_var]])
            }
    ...
    .plot = function(image, ...) {
      ...
      for (var in self$options$vars) {
        ...
        if (has_group) {
          ...
          x_axis_title <- paste(self$options$group, collapse = " & ")
