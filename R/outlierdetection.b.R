outlierDetectionClass <- R6::R6Class(
  ...
  private = list(
    ...
    .run = function() {
      ...
      # Mapeamento do valor numérico e do rótulo exato em texto (evita o R transformar 3.0 em 3)
      ...
      iqr_label <- label_map[[self$options$iqrMultiplier]]
      ...
      for (var in self$options$vars) {
        ...
        tryCatch({
          ...
          for (group_name in names(data_grouped)) {
            ...
            outlierLevel_full[outliers_indices_in_group] <- iqr_label # Usa o rótulo fixo "1.5", "2.2" ou "3.0"
      ...
      if (treatment_type != "none" &&
        ...
        for (var in names(all_treated_vectors)) {
          ...
          output_titles        <- c(output_titles,        paste0(var, suffix)) # Aplica o sufixo correto
      ...
      if (self$options$dispBox && length(all_plot_data) > 0) {
        ...
        image$setState(list(plot_data = all_plot_data, iqr_mult = iqr_mult, iqr_label = iqr_label))
    ...
    .plot = function(image, ...) {
      ...
      plot_list  <- list()
      iqr_label  <- image$state$iqr_label # Recupera o rótulo fixo ("3.0")
      group_vars <- self$options$group
