  ...
  private = list(
    ...
    .run = function() {
      ...
      for (var in self$options$vars) {
        ...
        tryCatch({
          ...
          for (group_name in names(data_grouped)) {
            ...
            outlierLevel_full[outliers_indices_in_group] <- as.character(threshold)
      ...
      if (self$options$dispBox && length(all_plot_data) > 0) {
        ...
        image$setState(list(plot_data = all_plot_data, threshold = threshold))
    ...
    .plot = function(image, ...) {
      ...
      for (var in self$options$vars) {
        ...
        active_levels <- c("Non-Outlier", as.character(threshold))
        ...
        legend_breaks <- as.character(threshold)
        legend_labels <- paste0("Outlier (|Z*| > ", threshold, ")")
