samplesizeClass <- R6::R6Class(
  "samplesizeClass",
  inherit = samplesizeBase,
  private = list(
    .run = function() {
      mode <- self$options$mode
      if (is.null(mode)) {
        return()
      }
      
      if (mode == "finite") {
        private$.runFinite()
      } else if (mode == "animals") {
        private$.runAnimals()
      }
    },
    
    .runFinite = function() {
      method <- self$options$method_finite
      if (is.null(method)) {
        return()
      }
      
      N <- self$options$population_size_finite
      confidence_level_pct <- self$options$confidence_level_finite
      margin_error_pct <- self$options$margin_error_finite
      p <- self$options$population_proportion_finite
      
      calculation_possible <- TRUE
      results_list <- list()
      
      if (is.null(confidence_level_pct) || is.null(margin_error_pct)) {
        jmvcore::reject("Confidence Level and Margin of Error must be provided.", code="missing_input_basic")
        calculation_possible = FALSE
      } else {
        confidence_level <- confidence_level_pct / 100
        margin_error <- margin_error_pct / 100
        
        if (confidence_level <= 0 || confidence_level >= 1) {
          jmvcore::reject("Confidence Level must be between 0 and 100.", code="invalid_confidence")
          calculation_possible = FALSE
        }
        if (margin_error <= 0 || margin_error >= 1) {
          jmvcore::reject("Margin of Error must be between 0 and 100.", code="invalid_error")
          calculation_possible = FALSE
        }
      }
      
      if (calculation_possible) {
        if (method == "Cochran") {
          if (is.null(N) || N <= 0) {
            jmvcore::reject("A valid Population Size (N > 0) must be provided for the Cochran (FPC) method.", code="missing_n_cochran_fpc")
            calculation_possible = FALSE
          } else if (is.null(p)) {
            jmvcore::reject("Population Proportion (p) must be provided for Cochran method.", code="missing_p_cochran")
            calculation_possible = FALSE
          } else if (p < 0 || p > 1) {
            jmvcore::reject("Population Proportion (p) must be between 0 and 1.", code="invalid_p_cochran")
            calculation_possible = FALSE
          } else if (p == 0 || p == 1) {
            jmvcore::reject("Population Proportion (p) cannot be exactly 0 or 1 for Cochran method.", code="extreme_p_cochran")
            calculation_possible = FALSE
          }
        } else if (method == "Slovin") {
          if (is.null(N) || N <= 0) {
            jmvcore::reject("A valid Population Size (N > 0) must be provided for Slovin method.", code="missing_n_slovin")
            calculation_possible = FALSE
          }
        }
      }
      
      if (calculation_possible) {
        sample_size <- NA_real_
        
        if (method == "Cochran") {
          z_value <- stats::qnorm(1 - (1 - confidence_level) / 2)
          n0 <- (z_value^2 * p * (1-p)) / (margin_error^2)
          
          if (n0 > 0) {
            sample_size <- n0 / (1 + (n0 - 1) / N)
            sample_size <- max(1, sample_size)
          } else {
            sample_size <- n0
          }
          
          results_list <- list(
            method = "Cochran",
            population_size = N,
            confidence_level = confidence_level_pct,
            margin_error = margin_error_pct,
            population_proportion = p,
            sample_size = ceiling(sample_size)
          )
          
        } else if (method == "Slovin") {
          sample_size <- N / (1 + N * (margin_error^2))
          sample_size <- max(1, sample_size)
          
          results_list <- list(
            method = "Slovin",
            population_size = N,
            confidence_level = confidence_level_pct,
            margin_error = margin_error_pct,
            population_proportion = NA,
            sample_size = ceiling(sample_size)
          )
        }
      }
      
      table <- self$results$sampleSizeTable_finite
      table$setRow(rowNo = 1, values = list())
      
      if (calculation_possible && length(results_list) > 0) {
        table$setRow(rowNo = 1, values = results_list)
      }
    },
    
    .runAnimals = function() {
      design <- self$options$design_animals
      if (is.null(design)) {
        return()
      }
      
      table <- self$results$sampleSizeTable_animals
      try(table$deleteRows(), silent = TRUE)
      
      rowIndex <- 1
      addParameterRow <- function(param, value) {
        table$addRow(rowKey = rowIndex, values = list(parameter = param, value = as.character(value)))
        rowIndex <<- rowIndex + 1
      }
      
      addParameterRow("Design Type", design)
      
      if (design == "Design 1 (One-way ANOVA)") {
        k <- self$options$nGroups_animals
        
        if (is.null(k) || k <= 0) {
          self$results$text_animals$setContent("Number of Groups (k) must be a positive integer for Design 1.")
          return()
        }
        
        min_n <- ceiling(10/k + 1)
        max_n <- floor(20/k + 1)
        if (max_n < min_n) max_n <- min_n
        
        min_total <- min_n * k
        max_total <- max_n * k
        
        addParameterRow("Number of Groups (k)", k)
        addParameterRow("Minimum Sample Size per Group", min_n)
        addParameterRow("Maximum Sample Size per Group", max_n)
        addParameterRow("Minimum Total Sample Size", min_total)
        addParameterRow("Maximum Total Sample Size", max_total)
        
        self$results$text_animals$setContent("Sample size calculation complete for Design 1 (One-way ANOVA).")
        
      } else if (design == "Design 2 (Repeated Measures)") {
        r <- self$options$nMeasures_animals
        sacrifice <- self$options$sacrifice_animals
        sacrifice_text <- if(sacrifice) "Yes" else "No"
        
        if (is.null(r) || r <= 1) {
          self$results$text_animals$setContent("Number of Repeated Measures (r) must be an integer greater than 1 for Design 2.")
          return()
        }
        
        base_min_n <- ceiling(10/(r-1) + 1)
        base_max_n <- floor(20/(r-1) + 1)
        if (base_max_n < base_min_n) base_max_n <- base_min_n
        
        if (sacrifice) {
          min_n <- base_min_n * r
          max_n <- base_max_n * r
          min_total <- min_n
          max_total <- max_n
        } else {
          min_n <- base_min_n
          max_n <- base_max_n
          min_total <- min_n
          max_total <- max_n
        }
        
        addParameterRow("Number of Repeated Measures (r)", r)
        addParameterRow("Animals Sacrificed", sacrifice_text)
        addParameterRow("Minimum Sample Size per Group", min_n)
        addParameterRow("Maximum Sample Size per Group", max_n)
        addParameterRow("Minimum Total Sample Size", min_total)
        addParameterRow("Maximum Total Sample Size", max_total)
        
        self$results$text_animals$setContent("Sample size calculation complete for Design 2 (Repeated Measures).")
        
      } else if (design == "Design 3 (Mixed ANOVA)") {
        k <- self$options$nGroups_animals
        r <- self$options$nMeasures_animals
        sacrifice <- self$options$sacrifice_animals
        sacrifice_text <- if(sacrifice) "Yes" else "No"
        
        if (is.null(k) || k <= 0 || is.null(r) || r <= 1) {
          self$results$text_animals$setContent("Number of Groups (k) must be a positive integer and Number of Repeated Measures (r) must be an integer greater than 1 for Design 3.")
          return()
        }
        
        denominator <- k * r
        
        base_min_n <- ceiling(10/denominator + 1)
        base_max_n <- floor(20/denominator + 1)
        
        if (base_max_n < base_min_n) {
          base_max_n <- base_min_n
        }
        
        if (sacrifice) {
          min_n <- base_min_n * r
          max_n <- base_max_n * r
          min_total <- min_n * k
          max_total <- max_n * k
        } else {
          min_n <- base_min_n
          max_n <- base_max_n
          min_total <- min_n * k
          max_total <- max_n * k
        }
        
        addParameterRow("Number of Groups (k)", k)
        addParameterRow("Number of Repeated Measures (r)", r)
        addParameterRow("Animals Sacrificed", sacrifice_text)
        addParameterRow("Minimum Sample Size per Group", min_n)
        addParameterRow("Maximum Sample Size per Group", max_n)
        addParameterRow("Minimum Total Sample Size", min_total)
        addParameterRow("Maximum Total Sample Size", max_total)
        
        self$results$text_animals$setContent("Sample size calculation complete for Design 3 (Mixed ANOVA).")
      }
    }
  )
)
